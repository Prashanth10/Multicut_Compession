...
UPDATED FOR BETTER COMPRESSION:
- Mask bbox to 0 for foreign pixels
- TINYREGIONSIZE = 5000
- Fixed threshold = -0.5
- Greedy compression-aware post-merge
"""
...
class Config:
    ...
    VAL_EVERY = 5
    TARGET_REGIONS = 200
    TINY_REGION_SIZE = 5000  # ← UPDATED: Increased from 1000 to 5000
...
def merge_tiny_regions(labels_2d, min_region_size=5000):
    """
...
# ============================================================================
# PNG COMPRESSION PER REGION (MASKED VERSION)
# ============================================================================

def encode_full_image_png(img_u8, labels_2d):
    """
    Encode image by compressing each region with PNG.
    UPDATED: Mask foreign pixels to 0 before PNG encoding
    """
    H, W, C = img_u8.shape
    ...
    for region_id in range(num_regions):
        ...
        # ← NEW: Mask foreign pixels to 0 (helps PNG compression!)
...
def encode_to_binary(encoded_data):
    """Convert encoded data to BINARY FORMAT."""
    buffer = io.BytesIO()
...
def decode_from_binary(binary_data):
    ...
    seg_rle_len = struct.unpack('<I', buffer.read(4))[0]
    seg_rle = buffer.read(seg_rle_len)
    ...
    for region_id in range(num_regions):
        try:
            y0, y1, x0, x1 = struct.unpack('<HHHH', buffer.read(8))

            mask_rle_len = struct.unpack('<I', buffer.read(4))[0]
            mask_rle = buffer.read(mask_rle_len)

            png_len = struct.unpack('<I', buffer.read(4))[0]
            png_data = buffer.read(png_len)
...
# ============================================================================
# NEW: GREEDY COMPRESSION-AWARE MERGE
# ============================================================================

def greedy_compress_merge(img_u8, labels_2d, max_iters=20):
    """
    Post-GAEC greedy merging: iteratively merge adjacent regions
    if doing so saves bytes.

    Strategy:
    1. Build adjacency of all regions
    2. For each pair of adjacent regions, test merge benefit:
       - Compute: size(merged) - size(A) - size(B) + segmentation_delta
    3. Pick best merge (most bytes saved)
    4. Repeat until no improvement
    """
    log(f"\n[GREEDY MERGE] Starting with {labels_2d.max() + 1} regions")

    H, W = labels_2d.shape
    labels_current = labels_2d.copy()

    for iteration in range(max_iters):
        num_regions = labels_current.max() + 1

        # Build adjacency
        adjacency = {r: set() for r in range(num_regions)}
        for y in range(H):
            for x in range(W):
                r = labels_current[y, x]
                # Check right neighbor
                if x < W - 1:
                    neighbor = labels_current[y, x + 1]
                    if neighbor != r:
                        adjacency[r].add(neighbor)
                        adjacency[neighbor].add(r)
                # Check down neighbor
                if y < H - 1:
                    neighbor = labels_current[y + 1, x]
                    if neighbor != r:
                        adjacency[r].add(neighbor)
                        adjacency[neighbor].add(r)

        # Compute current encoding
        current_encoded = encode_full_image_png(img_u8, labels_current)
        current_size = compute_compression_size_png(current_encoded)
        ...
        for r1 in range(num_regions):
            for r2 in adjacency[r1]:
                if r1 < r2 and (r1, r2) not in tested_pairs:
                    tested_pairs.add((r1, r2))

                    # Create merged labels
                    labels_merged = labels_current.copy()
                    labels_merged[labels_merged == r2] = r1

                    # Relabel consecutively
                    unique = np.unique(labels_merged)
                    remap = {old: new for new, old in enumerate(unique)}
                    labels_merged = np.array([remap[l] for l in labels_merged.flatten()]).reshape(H, W).astype(np.int32)

                    # Compute merged size
                    merged_encoded = encode_full_image_png(img_u8, labels_merged)
                    merged_size = compute_compression_size_png(merged_encoded)

                    saving = current_size - merged_size
        ...
        if best_saving > 0:
            log(f"[GREEDY MERGE] Iter {iteration+1}: Merged regions {best_pair}, saved {best_saving} bytes")
            labels_current = best_labels
        else:
            log(f"[GREEDY MERGE] No more beneficial merges found after {iteration+1} iterations")
            break

    final_regions = labels_current.max() + 1
    log(f"[GREEDY MERGE] Final: {final_regions} regions\n")
...
def gaec_additive(num_nodes, u_np, v_np, cost_np, merge_threshold=-0.5):
    """GAEC multicut solver."""
...
def find_optimal_threshold(edge_costs, target_regions, num_pixels):
    """Fixed aggressive threshold: -0.5"""
    if len(edge_costs) == 0:
        return -0.5
...
def inference_on_image(img_tensor, enc, pred, H, W, device):
    ...
    with torch.no_grad():
        ...
        # ← NEW: Greedy compression-aware merging
        labels_2d = greedy_compress_merge(img_u8, labels_2d, max_iters=20)
...
# TRAINING
def train_epoch(enc, pred, train_dl, opt, ep, u, v):
    enc.train()
    pred.train()
...
def main():
    ...
    log("="*70)
    log("MULTICUT COMPRESSION: UPDATED WITH COMPRESSION-AWARE OPTIMIZATIONS")
    log("="*70)
    log(f"TINYREGIONSIZE: {Config.TINY_REGION_SIZE}")
    log(f"Masked PNG: Enabled")
    log(f"Greedy merge: Enabled")
    log("="*70 + "\n")
    ...
    for ep in range(Config.EPOCHS):
        avg_loss = train_epoch(enc, pred, train_dl, opt, ep, u, v)
        scheduler.step()
